...
// conexión a 4 bits, cada que se genere una interrupción por cambio de      //
// estado en un pin del puerto B mientras no se genere la interrupción       //
// encendera y apagara un led cada 200ms.                                    //
///////////////////////////////////////////////////////////////////////////////
...
#define Enable RA0              // Nombramos el pin RB1 como Enable
#define RS RA1                  // Nombramos el pin RB2 como RS
#define Led RA2                 // Nombramos el pin RB3 como Led
...
void main(){
    ...
    TRISA = 248;                // Puerto A como salida en RA0, RA1 y RA2
    ANSELA = 0;                 // Puerto A digital
    TRISB = 15;                 // Puerto B salida excepto RB0, RB1, RB2 y RB3 como entradas para la interrupción
    ANSELB = 0;                 // Puerto B Digital
    PORTB = 0;                  // Limpio el puerto B

    // Configuración de interrupción
    INTCON = 136;               // Habilito las interrupciones y activo la interrupcion por cambio de bit en puerto
    IOCBN = 15;                 // Interrupción por flanco de bajada, en los pines RB0, RB1, RB2 y RB3.
...
void interrupt isr(void){
    irapos_lcd(64);                             // Nos ubicamos en la segunda linea
    cadena_lcd("ACTIVA EN! RB   ");             // Indicamos que la interrupción esta activa
    if(IOCBF == 1){                             // Si se activa la interrupción por RB0
        irapos_lcd(77);                         // Vamos a la posición para escribir donde se disparo
        cadena_lcd("0");                        // Escribimos el numero del bit
        __delay_ms(500);                        // Retarde de 500ms
    }else if(IOCBF == 2){                       // Si se activa la interrupción por RB1
        irapos_lcd(77);                         // Vamos a la posición para escribir donde se disparo
        cadena_lcd("1");                        // Escribimos el numero del bit
        __delay_ms(500);                        // Retarde de 500ms
    }else if(IOCBF == 4){                       // Si se activa la interrupción por RB2
        irapos_lcd(77);                         // Vamos a la posición para escribir donde se disparo
        cadena_lcd("2");                        // Escribimos el numero del bit
        __delay_ms(500);                        // Retarde de 500ms
    }else if(IOCBF == 8){                       // Si se activa la interrupción por RB3
        irapos_lcd(77);                         // Vamos a la posición para escribir donde se disparo
        cadena_lcd("3");                        // Escribimos el numero del bit
        __delay_ms(500);                        // Retarde de 500ms
    }
    irapos_lcd(64);                             // Nos ubicamos en la segunda linea
    cadena_lcd("  Inactiva...   ");             // Enviamos este mensaje de interrupción inactiva
    IOCBF = 0;                                  // Borro las banderas de interrupción en pin de un puerto
    IOCIF = 0;                                  // Pongo en 0 la bandera de interrupción.
}
