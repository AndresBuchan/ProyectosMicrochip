...
void main(){
    ...
    ANSELB = 2;                 // RB1 como entrada analoga
    ...
    while(1){                  // Programa que se ejecura continuamente
        ...
        pulso = ((conversion * 31)/1000)+8;     // Hago la operación para que se almacene en la variable
                                                // pulso el valor correspondiente a la posición del servo
                                                // minimo 8, maximo 39
        pulso_pwm(pulso);                       // Llamo la función que genera el ancho de pulso y le mando
